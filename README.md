...
- Risky sign-in investigation flows  
...
- Detection scripts (Teams, uninstallers, automations)  
- Hardening endpoints (USB restrictions, script blocking)  
- Edge bookmark management  
- System-context PowerShell execution  
- DeviceLogonEvents & MDE correlation  
- Outlook scheduled send troubleshooting  
...
- FileUploadedToCloud + DLP logic  
- Insider-risk analytics  
- USB exfiltration queries  
- Credential-theft detection (Mimikatz)  
- Identity logon correlation  
- Suspicious PowerShell/Node.js detections  
...
- Secure file erasure & encryption workflows  
- Nextcloud installations (Docker & native)  
- Cockpit management  
- Samba/NFS share setups  
- Cloudflared tunnel troubleshooting  
- GLIBC compatibility fixes  
- Cronjob automation examples  
- Server hardening practices  
- Frigate/Kerberos.io NVR setups  
...
- Docker stacks (Nextcloud, Zabbix, AdGuard, LAMP)  
- Cloudflare tunnels  
- S3 server backups  
- Arc-enabled servers  
- Plex/media server configurations  
- Proxmox & Hyper-V workloads  
...
- DNS/CAA/SPF/DKIM/DMARC settings  
- Encrypted container organization  
- Personal digital sovereignty setups  
